...
const terminals = new Map<string, { pty: pty.IPty; ws?: WebSocket }>();
...
const server = createServer(async (req, res) => {
  ...
  if (url === '/api/sessions' && method === 'GET') return json(res, { sessions: await getSessions() });
  const m = url.match(/^\/api\/sessions\/([^\/]+)/);
  if (m && method === 'GET') { const s = (await getSessions()).find(x => x.id === m[1]); return s ? json(res, s) : json(res, { error: 'Not found' }, 404); }
  if (m && method === 'POST' && url.includes('/terminal')) {
    if (terminals.has(m[1])) return json(res, { error: 'Running' }, 400);
    ...
    proc.onData(data => { const t = terminals.get(m[1]); if (t?.ws?.readyState === WebSocket.OPEN) t.ws.send(JSON.stringify({ type: 'output', data })); });
...
wss.on('connection', (ws, req) => {
  ...
  if (!term) { ws.send(JSON.stringify({ type: 'error', data: 'Not found' })); ws.close(); return; }
  term.ws = ws;
  ws.on('message', data => { try { const { type, data: input } = JSON.parse(data.toString()); if (type === 'input') term.pty.write(input); } catch { term.pty.write(data.toString()); } });
  ws.on('close', () => { const t = terminals.get(m[1]); if (t) t.ws = undefined; });
});
